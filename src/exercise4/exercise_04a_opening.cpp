...
int main(int argc, char *argv[])
{
    if (argc < 4)
    {
        cerr << "Error: number of arguments: " << argv[0] << " <opening size> <input image> <output image>" << endl;
        return EXIT_FAILURE;
    }

    // Read the opening size
    u_int OPENING_SIZE = atoi(argv[1]);
    ...
    u_int winNameLength = strlen("Window: image with opening size = ") + strlen(argv[1]) + 1;
